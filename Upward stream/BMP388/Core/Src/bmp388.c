...
uint8_t BMP388_ReadReg(uint8_t reg) {
    uint8_t tx = reg | 0x80;  // бит чтения = 1
    uint8_t rx = 0;
    SPI_CS_Low();
    HAL_SPI_TransmitReceive(spi_handle, &tx, &rx, 1, HAL_MAX_DELAY);
    tx=0xFF;
    HAL_SPI_TransmitReceive(spi_handle, &tx, &rx, 1, HAL_MAX_DELAY);
    SPI_CS_High();
    return rx;
...
static void BMP388_ReadRegs(uint8_t reg, uint8_t *buffer, uint8_t len) {
    uint8_t tx = reg | 0x80;
    SPI_CS_Low();
    HAL_SPI_Transmit(spi_handle, &tx, 1, HAL_MAX_DELAY);
    HAL_SPI_Receive(spi_handle, buffer, len, HAL_MAX_DELAY);
    SPI_CS_High();
}
...
static void BMP388_WriteReg(uint8_t reg, uint8_t data) {
    ...
    SPI_CS_Low();
    HAL_SPI_Transmit(spi_handle, tx, 2, HAL_MAX_DELAY);
    SPI_CS_High();
}
