...
int main(void)
{
  ...
  MX_USART1_UART_Init();
  /* USER CODE BEGIN 2 */
  Serial_Printf(&huart1, "\r\n--- Servo Control Only INIT ---\r\n");
  HAL_GPIO_WritePin(LED_PIN_1_GPIO_Port, LED_PIN_1_Pin, 1);

    // Инициализация PCA9685.
    // ВАЖНО: передаем 50 Гц, так как это стандарт для сервомоторов
    PCA9685_Init(50);

    if (HAL_I2C_IsDeviceReady(&hi2c1, PCA9685_I2C_ADDR << 1, 3, 100) == HAL_OK) {
        Serial_Printf(&huart1, "SUCCESS: PCA9685 is online!\r\n");
    } else {
        Serial_Printf(&huart1, "ERROR: PCA9685 NOT found on I2C!\r\n");
    }
  ...
  /* USER CODE BEGIN WHILE */
    // Переменные для отсчета времени без использования блокирующего HAL_Delay()
      uint32_t servo_timer = HAL_GetTick();
      uint8_t servo_step = 0;

      while (1)
      {
          // Проверяем, прошла ли 1 секунда (1000 мс)
          if (HAL_GetTick() - servo_timer >= 1000) {
              servo_timer = HAL_GetTick(); // Сбрасываем таймер
