...
HAL_StatusTypeDef BMI088_Init(SPI_HandleTypeDef *hspi) {
    ...
    status = BMI088_WriteReg(hspi, BMI088_CS_ACC_PIN_MCP, BMI088_ACC_CONF, 0xA8);
    ...
    status = BMI088_WriteReg(hspi, BMI088_CS_GYR_PIN_MCP, BMI088_GYR_BANDWIDTH, 0x02);
...
void BMI088_ToPhysical(const BMI088_Raw_t *raw, BMI088_Physical_t *phys) {
    ...
    // Применяем калибровку: вычитаем рассчитанное смещение нуля (ZRO)
